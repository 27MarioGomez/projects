...
# Si alguna cripto falla, retírala o revisa su ID en la API de CoinCap.
...
def load_coincap_data(coin_id, start_ms=None, end_ms=None, max_retries=3):
    """
    Descarga el histórico de precios desde CoinCap en intervalo diario (d1).
    ...
    """
    # Fijamos el intervalo a d1 (diario)
    ...
    for attempt in range(max_retries):
        ...
        if resp.status_code == 200:
            ...
            st.info("CoinCap: (400) Parámetros inválidos o rango excesivo. Reajusta rango de fechas.")
            return None
        else:
...
def train_and_predict(
    ...
    """
    - Descarga datos de CoinCap con (start_ms, end_ms) a intervalo diario (fijo).
    ...
    if df_prices is None or len(df_prices) == 0:
        st.warning("No se pudo descargar datos suficientes para entrenar. Reajusta rango de fechas.")
        return None
