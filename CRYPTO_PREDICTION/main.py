...
# Diccionarios con IDs de criptomonedas para CoinCap y mapeo a símbolo para LunarCrush
coincap_ids = {
    "Bitcoin (BTC)": "bitcoin",
...
# Mapeo de CoinCap id a símbolo (usado por LunarCrush)
coinid_to_symbol = {
    "bitcoin": "BTC",
    "ethereum": "ETH",
    "xrp": "XRP",
...
def build_lstm_model(input_shape, learning_rate=0.001):
    """
    Construye un modelo secuencial que combina Conv1D y Bidirectional LSTM para manejar precio + sentimiento.
    """
    model = Sequential()
...
def get_dynamic_params(df, horizon_days):
    """
    Calcula hiperparámetros dinámicos basados en la volatilidad y tamaño del dataset.
    """
    data_len = len(df)
...
##############################################
# Funciones para LunarCrush (versión Free)
##############################################

def get_crypto_sentiment_lunarcrush(symbol):
    """
    Obtiene el sentimiento de la criptomoneda 'symbol' usando el endpoint 'assets'
    de LunarCrush. Se prioriza 'galaxy_score' (0-100); si no está, se usa 'average_sentiment'
    (convertido de 0-1 a 0-100). Si falla, retorna 50.0.
    """
    ...
    try:
        ...
        avg_sent = asset.get("average_sentiment")
        if avg_sent is not None:
            return float(avg_sent) * 100.0
        return 50.0
...
def get_market_crypto_sentiment_lunarcrush():
    """
    Obtiene un sentimiento global aproximado del mercado cripto usando el endpoint 'market'
    de LunarCrush. En Free no hay un campo directo, así que se utiliza 'btc_dominance'
    como heurística (normalizada de 30 a 60% => 0 a 100). Si falla, retorna 50.0.
    """
    api_key = st.secrets["lunarcrush_api_key"]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data:
                ...
                # Normalizar: si btc_dominance=30 => 0 y 60 => 100
...
def get_lunarcrush_news(symbol, limit=5):
    """
    Obtiene noticias recientes para la criptomoneda 'symbol' usando el endpoint 'news'
    de LunarCrush. Si no hay datos (o en Free están limitados), retorna una lista vacía.
    """
    api_key = st.secrets["lunarcrush_api_key"]
...
##############################################
# Entrenamiento y predicción con LSTM (con sentimiento LunarCrush)
##############################################
def train_and_predict_with_sentiment(coin_id, use_custom_range, start_ms, end_ms, horizon_days=30, test_size=0.2):
    """
    Descarga datos de CoinCap, obtiene sentimiento (cripto y mercado) desde LunarCrush,
    entrena el modelo LSTM integrando ese sentimiento y realiza predicciones.
    """
    ...
    # Convertir coin_id a símbolo para LunarCrush
    ...
    # Factor combinado (normalizado de 0 a 1)
    sentiment_factor = (crypto_sent + market_sent) / 200.0

    st.write(f"Sentimiento de {symbol}: {crypto_sent:.2f} (0-100)")
    st.write(f"Sentimiento de mercado: {market_sent:.2f} (0-100)")
    st.write(f"Factor combinado (0-1): {sentiment_factor:.2f}")

    # Calcular hiperparámetros
    window_size, epochs, batch_size, learning_rate = get_dynamic_params(df, horizon_days)
    st.info(f"Hiperparámetros: window_size={window_size}, epochs={epochs}, batch_size={batch_size}, lr={learning_rate}")
    ...
    # Ajustar para que cada secuencia tenga dos características: precio y sentimiento
    ...
    st.write(f"Dimensión X_train ajustada: {X_train_adj.shape}")
...
def main_app():
    ...
    st.markdown("**Fuente de Datos:** CoinCap (precios) y LunarCrush (sentimiento y noticias)")
    ...
    tabs = st.tabs(["🤖 Entrenamiento y Test", f"🔮 Predicción de Precios", "📰 Noticias"])
    ...
    with tabs[2]:
        ...
        if 'result' in locals() and result is not None:
            # Usamos el símbolo ya obtenido en la fase de sentimiento
            ...
            if news_items:
                ...
                st.write("No se encontraron noticias o los datos están limitados en el plan Free.")
        else:
            st.info("Primero entrena el modelo para mostrar noticias relacionadas.")
